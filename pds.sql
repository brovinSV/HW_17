...
create database wrpracti_bookinfo;
...
# Task1
/*
 Напишіть запит, щоб перерахувати кількість вакансій,
 доступних у таблиці employees.
 */
SELECT COUNT(DISTINCT `JOB_ID`) as `number_of_vacancies` from `employees` 

# Task2
/*
 Напишіть запит, щоб отримати середню заробітну плату та кількість
 працівників відділу 90.*/
SELECT ROUND(AVG(CAST(`SALARY` AS DOUBLE)), 2) as 'avg', 
       COUNT(`EMPLOYEE_ID`) AS `amount of workers`
FROM `employees`
WHERE `DEPARTMENT_ID` = 90 

# Task3
/*
 Напишіть запит, щоб отримати кількість працівників з тією
 самою роботою.*/
 SELECT `JOB_ID`, COUNT(*) AS num_jobs FROM `employees`
 GROUP BY `JOB_ID`

# Task4
/*
 Напишіть запит, щоб знайти ім'я (ім'я, прізвище), код відділу та
 імена всіх співробітників.
 */
...
# Task5
/*
 Напишіть запит, щоб знайти ім'я (ім'я, прізвище), роботу, ідентифікатор
 відділу та імена співробітників, які працюють у Лондоні.*/
SELECT  `FIRST_NAME`, 
        `LAST_NAME`,
        `JOB_ID`,
        `DEPARTMENT_ID`
